  ...
  async initializeDefaultProviders(): Promise<void> {
    ...
    if (liteLLMBaseUrl) {
      try {
        ...
        console.log('✅ Initialized LiteLLM provider (local)');
      } catch (error) {
        console.error('❌ Failed to initialize LiteLLM provider:', error);
      }
    } else {
      console.warn('⚠️  LITELLM_BASE_URL not found - LiteLLM provider not initialized');
    }
